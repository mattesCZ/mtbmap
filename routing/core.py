...
def create_gpx(points):
    ...
    trkseg_node = libxml2.newNode('trkseg')
    for point in points:
        rtept = libxml2.newNode('trkpt')
        rtept.setProp('lat', str(point[0]))
        rtept.setProp('lon', str(point[1]))
        trkseg_node.addChild(rtept)
    trk_node = libxml2.newNode('trk')
    trk_node.addChild(trkseg_node)
    root_node.addChild(trk_node)
