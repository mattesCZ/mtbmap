...
import datetime, re, httplib, string
...
if __name__ == "__main__":
    ...
    try:
        try:
            confFile = open('default.json')
            configuration = json.load(confFile)
            variables = configuration[0]
            boundingBox = configuration[1]
        except IOError:
            raise UpdateError('Cannot open JSON configuration file')
        try:
