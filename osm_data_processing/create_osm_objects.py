...
def copy_osmpoints():
    ...
    query = ("INSERT INTO osm_data_processing_osmpoint (osm_id, the_geom, %s) SELECT osm_id, ST_TRANSFORM(way, 4326)"
             " as the_geom, %s FROM planet_osm_point WHERE %s;" % (columns, substr_columns, or_clause))
    logger.info('Copy SQL query: %s' % query)
...
def copy_osmlines():
    ...
    logger.debug('OSM column names: %s' % column_names)
    logger.debug('Routing column names: %s' % columns)
    substr_columns = ', '.join(['substr("' + column + '", 0, 200) as "' + column + '" ' for column in column_names])
    or_clause = ' OR '.join(['"' + column + '" IS NOT NULL' for column in column_names])
    query = ("INSERT INTO osm_data_processing_osmline (osm_id, the_geom, %s) SELECT osm_id, ST_TRANSFORM(way, 4326)"
             " as the_geom, %s FROM planet_osm_line WHERE %s;" % (columns, substr_columns, or_clause))
    logger.info('Copy SQL query: %s' % query)
    cursor.execute(query)
    transaction.commit_unless_managed(using=MAP_DB)
    cursor.close()
...
def osm_tag_names(obj):
    ...
    return [osm_tag_name(name) for name in names]
